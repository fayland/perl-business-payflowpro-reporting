...
sub new {
    ...
    defined $args->{password} or do { $errstr = 'password is required'; return };
    
    if ($args->{test_mode}) {
        $args->{url} = 'https://payments-reports.paypal.com/test-reportingengine';
    } else {
...
sub runReportRequest {
    ...
    if (exists $d->{runReportResponse}->{reportId}) {
        ...
        $errstr = $d->{runReportResponse}->{statusMsg} || $d->{baseResponse}->{responseMsg};
        return;
    }
}
...
sub getResultsRequest {
    ...
    if ($d->{baseResponse}->{responseCode} == 100) {
        ...
        $errstr = $d->{getResultsResponse}->{Results}->{statusMsg} || $d->{baseResponse}->{responseMsg};
        return;
    }
}
...
sub getDataRequest {
    ...
    if ($d->{baseResponse}->{responseCode} == 100) {
        ...
        my @reportDataRow = ref($reportDataRow) eq 'ARRAY' ? @$reportDataRow : ($reportDataRow);
